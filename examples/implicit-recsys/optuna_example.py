...
valid_evaluator = fastmf.evaluator.Evaluator(Y_valid, Y_train, k=5, metrics=["DCG"])
test_evaluator = fastmf.evaluator.Evaluator(Y_test, Y_train, k=5)
...
def expomf_objective(trial: optuna.Trial):
    ...
    weight_decay = trial.suggest_loguniform("weight_decay", 1e-4, 1e-1)
    model = fastmf.ExpoMF(num_components=args.num_components, lam_y=weight_decay, weight_decay=weight_decay)
...
def wmf_objective(trial: optuna.Trial):
    ...
    model = fastmf.WMF(num_components=args.num_components, learning_rate=alpha, weight_decay=weight_decay, weight=weight)
    model.fit(dataset.train, num_iterations=iterations, num_threads=args.threads, verbose=False)
    return valid_evaluator.evaluate(model.W@model.H.T)["DCG@5"]
...
print(study.best_params)
result = []
model = fastmf.ExpoMF(num_components=args.num_components, lam_y=study.best_params["weight_decay"],weight_decay=study.best_params["weight_decay"])
...
print(study.best_params)
result = []
model = fastmf.WMF(num_components=args.num_components, learning_rate=study.best_params["alpha"], weight_decay=study.best_params["weight_decay"], weight=study.best_params["weight"])
model.fit(dataset.train, num_iterations=study.best_params["iterations"], num_threads=args.threads, verbose=False)
for i in range(5):
    result.append(test_evaluator.evaluate(model.W @ model.H.T))
